...
function invitationStatus(invitation: CampaignInvitation): 'accepted' | 'revoked' | 'expired' | 'active' {
...
export function MembersPage() {
  ...
  const [createdInvitation, setCreatedInvitation] = useState<CreatedCampaignInvitation>();
  ...
  const members = useQuery({
    ...
    queryFn: () => api.request<CampaignMember[]>(`/campaigns/${campaignId}/members`),
    enabled: Boolean(campaignId) && isOwner,
    retry: false,
  });
  const invitations = useQuery({
    queryKey: ['invitations', campaignId],
    queryFn: () => api.request<CampaignInvitation[]>(`/campaigns/${campaignId}/invitations`),
    enabled: Boolean(campaignId) && isOwner,
    retry: false,
  });
  const addMember = useMutation({
    mutationFn: (payload: { email: string; role: string }) => api.request<CampaignMember>(`/campaigns/${campaignId}/members`, {
      method: 'POST',
  ...
  const updateMember = useMutation({
    mutationFn: ({ userId, role }: { userId: string; role: string }) => api.request<CampaignMember>(`/campaigns/${campaignId}/members/${userId}`, {
      method: 'PATCH',
  ...
  const removeMember = useMutation({
    mutationFn: (userId: string) => api.request<void>(`/campaigns/${campaignId}/members/${userId}`, { method: 'DELETE' }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['members', campaignId] });
      setError(undefined);
    },
  ...
  const createInvitation = useMutation({
    mutationFn: (role: string) => api.request<CreatedCampaignInvitation>(`/campaigns/${campaignId}/invitations`, {
      method: 'POST',
      body: JSON.stringify({ role }),
    }),
    onSuccess: (invitation) => {
      ...
      setError(undefined);
      void queryClient.invalidateQueries({ queryKey: ['invitations', campaignId] });
    },
    onError: (cause) => setError(apiErrorMessage(cause, t)),
  });
  const revokeInvitation = useMutation({
    mutationFn: (invitationId: string) => api.request<void>(`/campaigns/${campaignId}/invitations/${invitationId}`, { method: 'DELETE' }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['invitations', campaignId] });
      setError(undefined);
    },
  ...
  function submitMember(event: FormEvent<HTMLFormElement>) {
    ...
    addMember.mutate({ email: String(form.get('email') ?? ''), role: String(form.get('role') ?? 'PLAYER') });
    event.currentTarget.reset();
  }

  function submitInvitation(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    createInvitation.mutate(String(new FormData(event.currentTarget).get('role') ?? 'PLAYER'));
  }

  async function copyInvitation() {
    if (!createdInvitation) return;
    try {
      await navigator.clipboard.writeText(`${window.location.origin}/invitations/${createdInvitation.token}`);
    } catch {
  ...
  return (
    <main className="app-page">
      ...
      {members.isLoading || invitations.isLoading || campaign.isLoading ? <p>{t('common.loading')}</p> : (
        <div className="members-layout">
          ...
          <section className="panel">
            ...
            <form onSubmit={submitInvitation}>
              <RoleSelect />
              <button disabled={createInvitation.isPending}>{t('members.createInvitation')}</button>
            </form>
            {createdInvitation && (
              <div className="created-invitation" role="status">
                ...
                <button className="button-ghost" type="button" onClick={() => void copyInvitation()}>{t('members.copyInvitation')}</button>
              </div>
            )}
          </section>
                ...
                {members.data.map((member) => (
                  <article className="member-row" key={member.memberId}>
                    ...
                    <div className="action-row">
                      <select
                        aria-label={t('members.roleFor', { name: member.user.name || member.user.email })}
                        defaultValue={member.campaignRole}
                        onChange={(event) => updateMember.mutate({ userId: member.user.userId, role: event.target.value })}
                      >
                        {roles.map((role) => <option key={role} value={role}>{t(`workspace.roles.${role}`)}</option>)}
                      </select>
                      <button className="button-danger" type="button" onClick={() => {
                        if (window.confirm(t('members.removeConfirmation', { name: member.user.name || member.user.email }))) removeMember.mutate(member.user.userId);
                  ...
                  return (
                    <article className="member-row" key={invitation.invitationId}>
                      <div>
                        ...
                        <p>{t('members.expiresAt', { date: dateFormat.format(new Date(invitation.expiresAt)) })}</p>
                        <small className={`invitation-status invitation-${status}`}>{t(`members.statuses.${status}`)}</small>
                      </div>
                      {status === 'active' && <button className="button-danger" type="button" onClick={() => {
                        if (window.confirm(t('members.revokeConfirmation'))) revokeInvitation.mutate(invitation.invitationId);
                      }}>{t('members.revoke')}</button>}
                    </article>
                  );
                })}
              </div>
